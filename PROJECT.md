...
- **卡背随机分配 + 瑕疵**：卡背用用户提供的 8 张图（`public/cards/back1..8.jpg`，1080×1614），洗牌时给每张牌**随机分配**一张（`backIndex = Math.floor(Math.random()*8)`，存在 `DeckCard.backIndex`）。每张牌再用 `hashSeed(card.id)`（FNV-1a）做确定性 PRNG 生成细微差异：图案**旋转偏移 ±2°**、**亮度/对比度/饱和度微差**、**纸张颗粒噪点**（SVG `feTurbulence` + `mix-blend-mode: overlay`）+ 边缘暗角，模拟一副被反复使用过的真牌；选中高亮为暖金色光晕。
