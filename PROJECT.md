...
| 模式 | 说明 |
|---|---|
| **🃏 选牌模式** | 左侧一条可滚动的纵列牌库，顶部 5 个分类 tab（大阿卡纳 / 权杖 / 圣杯 / 宝剑 / 星币）。**点选一张牌 → 点牌阵空位放置**，点已放牌可移除，点同一张牌可切换正/逆位。放满后点「开始解读」。 |
| **⌨️ 快速输入** | 原文字符号输入模式（`0` / `四4` / `-三10`…），下方实时预览。 |

**选牌模式交互细节**（`PickMode.tsx` + `SpreadBoard.tsx`）：
- 牌阵每个位置渲染成可点击的 `slot`：空位是虚线占位框，选中某张牌后空位会淡显该牌的预览图（逆位则倒置）。
- 选牌态：点选卡牌高亮，右上角标「正/逆」；**再点同一张牌切换正逆位**。
- 放置后清空选中态；点已放牌移除回空。
- 牌数未满时「开始解读」禁用，按钮显示 `（已放 X / 总 N）`。
- 切换牌阵时清空已放牌与选中态。
- 选牌模式下放置的牌同样走 `makeParsedCard()` 构造成 `ParsedCard`，与快速输入共用解读链路。

> 注：未采用 HTML5 原生拖拽——微信手机端拖拽不可靠，改为「点选 + 点放」交互更稳。
...
│   │   ├── PickMode.tsx        # 选牌模式（左侧分类列 + 点选放置）
│   │   ├── SpreadBoard.tsx     # 选牌模式的可点击牌阵位（空位/已放）
