...
| 拖拽 | @dnd-kit/core | 6.x | 选牌模式拖放（触屏长按 + 鼠标移动激活） |
...
**问题输入框（三模式共用）**：三种模式上方共用一个可选的「💭 你想问的问题」输入框，输入的问题会在解读时一并传给 AI。链路：`App.tsx` 的 `question` state → `handleReadingStart` 调 `startReading(spread, cards, question)` → `POST /api/reading` body 带 `question` → `worker.js` 的 `buildUserPrompt` 在牌阵前插入「**你的问题**：…」段。

**选牌模式交互细节**（`PickMode.tsx`，基于 @dnd-kit）：
- **拖放**：电脑端鼠标按住拖动即激活；手机端**长按 ~350ms** 激活拖动（之前可正常滚动左侧牌库）。拖动时有一张浮空牌图跟随指针，松手放到空位即放置，放到已放牌上即替换。
- **点选**：点选一张牌高亮，再点空位放置（备用交互，也方便触屏）。
- **正逆位**：右侧一个「正位 / 逆位」分段开关，决定下一张放置牌的朝向（拖放与点选共用）。
- 牌阵每个位置渲染成 `useDroppable` 的 slot：空位是虚线框，拖拽悬停时高亮并显示「放置」，选中牌后空位淡显预览图。
- 牌数未满时「开始解读」禁用，按钮显示 `（已放 X / 总 N）`；切换牌阵时清空已放牌与选中态。
...
| 2026-08-13 | **功能**：三种模式（选牌/抽牌/快速输入）上方新增**可选问题输入框**，用户问题随解读请求传给 AI（`buildUserPrompt` 在牌阵前插入「你的问题」段）。 |
