...
- **真实牌背 + 瑕疵**：卡背用 RWS 经典牌背「Roses and Lilies」（`public/cards/back.jpg`，Wikimedia Commons 850×1451，美国公有领域）。每张牌用 `hashSeed(card.id)`（FNV-1a）做确定性 PRNG 生成细微差异：图案**旋转偏移 ±2°**、**亮度/对比度/饱和度微差**、**纸张颗粒噪点**（SVG `feTurbulence` + `mix-blend-mode: overlay`）+ 边缘暗角，模拟一副被反复使用过的真牌；选中高亮改为暖金色光晕。
